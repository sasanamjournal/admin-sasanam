...
export default function SubscriptionPayments() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['sub-payments', page, status, search],
    queryFn: () =>
      getSubscriptionPayments({ page, limit: 20, status, search }).then((r) => r.data.data),
  })

  const refundMut = useMutation({
    mutationFn: ({ paymentId }: { paymentId: string }) => refundPayment(paymentId, 'subscription'),
    onSuccess: () => {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-serif font-black text-body">Subscription Payments</h1>
        <p className="text-sm text-muted mt-1">All subscription payment transactions</p>
      </div>
          ...
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-primary/10 bg-cream/50">
                    ...
                    <th className="px-4 py-3 text-center text-2xs font-black uppercase tracking-widest text-primary/70">Status</th>
                ...
                <tbody>
                  {data?.payments?.map((p: any) => (
                    <tr key={p._id} className="border-b border-primary/5 hover:bg-white/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center font-black text-body">₹{(p.amount).toFixed(2)}</td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-2xs font-black uppercase tracking-wider ${statusBadge(p.status)}`}>
                          {p.status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        {p.status === 'paid' && p.paymentId && (
                          <button
                            onClick={() => {
                              if (confirm(`Refund ₹${p.amount} for order ${p.orderId}?`))
            ...
            {data && data.totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-primary/10">
                <p className="text-xs text-muted">Page {data.page} of {data.totalPages} ({data.total} payments)</p>
