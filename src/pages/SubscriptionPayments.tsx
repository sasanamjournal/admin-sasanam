...
export default function SubscriptionPayments() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-serif font-black text-[#4A3B32]">Subscription Payments</h1>
        <p className="text-sm text-[#6A5A4A] mt-1">All subscription payment transactions</p>
      </div>
                  ...
                  {data?.payments?.map((p: any) => (
                    <tr key={p._id} className="border-b border-[#8B4513]/5 hover:bg-white/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${statusBadge(p.status)}`}>
                          {p.status}
                        </span>
                      </td>
