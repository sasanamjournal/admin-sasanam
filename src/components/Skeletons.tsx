  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-card rounded-2xl p-5 border border-white/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-card rounded-2xl p-6 border border-white/30">
          <Pulse className="h-4 w-40 mb-6" />
          <Pulse className="h-[280px] w-full rounded-xl" />
        </div>
        <div className="bg-card rounded-2xl p-6 border border-white/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-card rounded-2xl p-6 border border-white/30">
...
export function TableSkeleton({ rows = 8, cols = 5 }: { rows?: number; cols?: number }) {
  return (
    <div className="bg-card rounded-2xl border border-white/30 shadow-sm overflow-hidden">
      {/* Header row */}
      <div className="flex gap-4 px-4 py-3 border-b border-primary/10 bg-cream/50">
        {[...Array(cols)].map((_, i) => (
      ...
      {/* Rows */}
      {[...Array(rows)].map((_, i) => (
        <div key={i} className="flex items-center gap-4 px-4 py-4 border-b border-primary/5">
      ...
      {/* Pagination */}
      <div className="flex justify-between px-4 py-3 border-t border-primary/10">
...
export function CardGridSkeleton({ count = 6, cols = 3 }: { count?: number; cols?: number }) {
  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 ${colClass} gap-5`}>
      {[...Array(count)].map((_, i) => (
        <div key={i} className="bg-card rounded-2xl p-6 border border-white/30 flex flex-col items-center">
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="rounded-2xl border-2 border-border/40 p-5">
            <Pulse className="h-5 w-24 mb-2" />
            <Pulse className="h-3 w-40" />
          </div>
        ))}
      </div>
      <div className="bg-card rounded-2xl border border-white/30 overflow-hidden">
        <div className="px-5 py-4 border-b border-primary/10 bg-cream/50">
          <Pulse className="h-4 w-44" />
        </div>
        {[...Array(10)].map((_, i) => (
          <div key={i} className="flex items-center gap-4 px-5 py-3 border-b border-primary/5">
